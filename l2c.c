...
// ----------------------------------------------------------------------------
// Minimal API for Python wrapper

char* l2c_generate(const char* checkpoint, const char* prompt, int steps,
    ...
    ssize_t file_size;     // size of the checkpoint file in bytes
    {
        ...
        if (fread(&config, sizeof(Config), 1, file) != 1) { return NULL; }
    ...
    size_t out_capacity = steps * (max_token_length + 1) + 1;
    ...
    while (pos < steps) {
        ...
        if(out_len + tlen < out_capacity) {
            memcpy(out + out_len, token_str, tlen);
            out_len += tlen;
            out[out_len] = '\0';
        }
        token = next;
...
void l2c_free(char* ptr) { free(ptr); }
